...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X SA" :jurisdiction "NIC"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "NIC" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:contract-value-cordobas (:engagement m)) (:contract-value-cordobas (:engagement d))))
    (is (= (:claimed-certificado-requerido? (:engagement m)) (:claimed-certificado-requerido? (:engagement d))))
