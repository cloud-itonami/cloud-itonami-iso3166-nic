  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Managua Inversiones del Pacífico S.A."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Managua Inversiones del Pacífico S.A." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest certificado-verificacion-claim-mismatch-is-held-and-unoverridable
  (testing "declared :claimed-certificado-requerido? does not match the independently recomputed Ley 1238 Art.18.6/Art.29.4 threshold -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:certificado-verificacion-claim-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest certificado-verificacion-unverified-is-held-and-unoverridable
  (testing "unverified Certificado de Verificación de Proveedores del Estado when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:certificado-verificacion-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
