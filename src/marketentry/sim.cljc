...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (NIC, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up certificado-verificacion-claim-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (certificado-verificacion-claim-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up certificado-verificacion-unverified) ==")
    ...
    (println "== filing/submit eng-5 (certificado-verificacion-unverified -> HARD hold) ==")
