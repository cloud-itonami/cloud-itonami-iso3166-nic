(ns marketentry.marketentryllm
  ...
  the Republic of Nicaragua (NIC) public-sector market-entry compliance
  actor.
  ...
  cited), never a committed record or a real DGCE filing / Registro
  Público Mercantil inscription. Every output is censored downstream by
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world DGCE filing / Registro
  Público Mercantil inscription request."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けDGCE提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "contract-value-cordobas=" (:contract-value-cordobas e)
                        " claimed-certificado-requerido?=" (:claimed-certificado-requerido? e)
                        " certificado-verificado?=" (:certificado-verificado? e)
     ...
     :confidence (if (and e
                          (or (not (:claimed-certificado-requerido? e))
                              (:certificado-verificado? e)))
                   0.9 0.3)}))
